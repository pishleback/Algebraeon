...
use crate::golay_codes::ordered_syntheme::{
...
use algebraeon_rings::{
    ...
    linear::{
        ...
        const_galois_monomial_transformation::ConstSizeGaloisMonomialTransformation,
...
impl HexacodeVector {
    pub fn is_hexacodeword(&self) -> bool {
        hexacode_structure().is_element(&self.0)
    }
}
...
pub trait OrderedSynthemeGaloisAutomorphism {
    /// Does this automorphism of F4^6 preserve the hexacode?
    ///
    /// The subgroup of pure permutation automorphisms is isomorphic to S4 and is generated by:
    ///  - Any permutation of the 3 pairs, preserving the order within each pair
    ///  - Transposing the elements within 2 of the pairs
    ///
    /// Any scalar multiplication is an automorphism.
    ///
    /// The only transformations with trivial permutation part which are automorphisms are those where all scalar parts are equal
    ///
    /// The subgroup of monomial transformations is isomorphic to C3.A6 and is generated by:
    ///  - Alternating permutation of the 6 points
    ///  - Scalar multiplications
    ///
    /// The full group of galois monomial transformations is isomorphic to C3.S6 and is generated by:
    ///  - Alternating permutation of the 6 points
    ///  - Scalar multiplications
    ///  - The galois automorphism and a transposition of 2 points
    fn is_hexacode_automorphism(&self) -> bool;
}

impl OrderedSynthemeGaloisAutomorphism
    for ConstSizeGaloisMonomialTransformation<6, OrderedSynthemePoint, QuaternaryField, C2>
{
    fn is_hexacode_automorphism(&self) -> bool {
        for vector in hexacode_structure().list_all_elements() {
            if !hexacode_ambient_space_structure()
                .submodules()
                .contains_element(
                    hexacode(),
                    &hexacode_ambient_space_structure()
...
mod tests {
    ...
    #[test]
    fn test() {
        let basis_perms = hexacode_ambient_space_structure()
            .basis_set()
            .const_size_permutations();
        let space_aut = hexacode_ambient_space_structure().galois_monomial_transformations();
        ...
        assert!(!aut1.is_hexacode_automorphism());
        assert!(!aut2.is_hexacode_automorphism());
        assert!(aut3.is_hexacode_automorphism());
        assert!(aut2.compose(&aut1).is_hexacode_automorphism());
    }
    ...
    fn test_is_hexacodeword() {
        ...
        let mut c = 0;
        for x in hexacode_ambient_space_structure()
            .list_all_elements()
            .into_iter()
            .map(HexacodeVector)
        {
