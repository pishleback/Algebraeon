...
## Combined Additive and Multiplicative Structure
 ...
 In that case the solution (or lack thereof) to \\(a = b \times x\\) for \\(x\\) given \\(a\\) and non-zero \\(b\\) is unique whenever it exists and is computed using `.try_left_divide(a: X, b: X) -> Option<X>`. `None` is also returned if \\(b = 0\\).
 ...
 In that case the solution (or lack thereof) to \\(a = x \times b\\) for \\(x\\) given \\(a\\) and non-zero \\(b\\) is unique whenever it exists and is computed using `.try_right_divide(a: X, b: X) -> Option<X>`. `None` is also returned if \\(b = 0\\).
