...
use crate::geometry_old::vector::Vector;
use orthoclase_rings::linear::{matrix::Matrix, subspace::AffineLatticeElements};
    ...
    match cut_simplex.affine_span().elems() {
        ...
        AffineLatticeElements::NonEmpty { offset, linlat } => {
            ...
            for linhyperplane in linhyperplanes {
                ...
                for c in 0..dim - 1 {
                    pts.push(&offset_pt + &Vector::from_matrix(&linhyperplane.basis_matrix(c)));
                }
            ...
            let mat =
                Matrix::join_cols(dim, (0..flat_dim).map(|i| linlat.basis_matrix(i)).collect());
            ...
            if flat_dim != 0 {
                ...
                let flat_transform = &|pt: Vector| {
                    Vector::from_matrix(&mat.col_solve(&(&pt - &offset_vec).as_matrix()).unwrap())
                };
                ...
                let unflat_transform = &|pt: Vector| {
                    &Vector::from_matrix(&Matrix::mul(&mat, &pt.as_matrix()).unwrap()) + &offset_vec
                };
