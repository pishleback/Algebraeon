...
#[derive(Debug, Clone)]
pub struct EmbeddedAffineSubspace<FS: OrderedRingSignature + FieldSignature> {
    // The ordered_field of ambient_space and subspace must match
    ambient_space: AffineSpace<FS>,
    embedded_space: AffineSpace<FS>,
    /*
    ...
    */
    embedding_points: Vec<Vector<FS>>,
}
...
impl<FS: OrderedRingSignature + FieldSignature> EmbeddedAffineSubspace<FS> {
    ...
    ) -> Result<(Self, Vec<Vector<FS>>), &'static str> {
        for point in &points {
...
impl<FS: OrderedRingSignature + FieldSignature> EmbeddedAffineSubspace<FS> {
    ...
    pub fn embedded_space(&self) -> &AffineSpace<FS> {
        &self.embedded_space
    }
    ...
    pub fn get_root_and_span(&self) -> Option<(Vector<FS>, Vec<Vector<FS>>)> {
    ...
    pub fn get_embedding_points(&self) -> &Vec<Vector<FS>> {
        &self.embedding_points
    }

    pub fn embed_point(&self, pt: &Vector<FS>) -> Vector<FS> {
    ...
    pub fn embed_simplex(&self, spx: &Simplex<FS>) -> Simplex<FS> {
        self.ambient_space()
            .simplex(spx.points().iter().map(|p| self.embed_point(p)).collect())
            .unwrap()
    }

    pub fn unembed_point(&self, pt: &Vector<FS>) -> Option<Vector<FS>> {
        ...
        match self.get_root_and_span() {
            Some((root, span)) => {
                ...
                Some(self.embedded_space().clone().vector(x?))
            }
            None => None,
        }
    }

    pub fn unembed_simplex(&self, spx: &Simplex<FS>) -> Option<Simplex<FS>> {
        let mut pts = vec![];
    ...
    pub fn to_oriented_hyperplane_intersection(&self) -> Option<Vec<OrientedHyperplane<FS>>> {
    ...
    pub fn to_oriented_hyperplane(&self) -> Option<OrientedHyperplane<FS>> {
...
pub fn compose_affine_embeddings<FS: OrderedRingSignature + FieldSignature>(
    _a_to_b: EmbeddedAffineSubspace<FS>,
    _b_to_c: EmbeddedAffineSubspace<FS>,
) -> EmbeddedAffineSubspace<FS> {
