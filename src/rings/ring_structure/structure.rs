...
pub trait RingStructure: EqualityStructure {
    ...
    fn sum(&self, vals: Vec<&Self::Set>) -> Self::Set {
        let mut sum = self.zero();
        for val in vals {
            self.add_mut(&mut sum, val);
        }
        sum
    }
    ...
    fn product(&self, vals: Vec<&Self::Set>) -> Self::Set {
        let mut prod = self.one();
        for val in vals {
            self.mul_mut(&mut prod, val);
        }
        prod
    }
...
pub trait EuclideanDivisionStructure: IntegralDomainStructure {
    ...
    where
        Self: GreatestCommonDivisorStructure,
    {
        let orig_x = x.clone();
        let orig_y = y.clone();
...
impl<FS: FieldOfFractionsStructure> CharZeroStructure for FS
where
    FS::RS: CharZeroStructure,
{
