...
use crate::{
    ...
    rings::{
        ...
        ring_structure::quotient::QuotientStructure,
        structure::{CannonicalStructure, StructuredElement},
    },
};
...
impl<const N: usize> TryFrom<super::super::permutation::Permutation> for Permutation<N> {
    ...
    fn try_from(mut value: super::super::permutation::Permutation) -> Result<Self, Self::Error> {
        let n = value.n();
        if N < n {
            Err(())
        } else {
...
impl<const N: usize> Permutation<N> {
    ...
    pub fn new_from_modular_func(f: impl Fn(Modulo<N>) -> Modulo<N>) -> Result<Self, &'static str> {
        let mut perm = [0; N];
        for i in (0..N) {
