...
pub trait CompositionSignature: SetSignature {
    ...
    fn compose_mut(self: &Rc<Self>, a: &mut Self::Elem, b: &Self::Elem) {
        *a = self.compose(a, b);
    }
}
...
pub trait AssociativeCompositionSignature: CompositionSignature {
    ...
    fn compose_nonempty_list(
        self: &Rc<Self>,
        mut elems: Vec<impl Borrow<Self::Elem>>,
    ) -> Option<Self::Elem> {
...
{
    fn try_difference(self: &Rc<Self>, a: &Self::Elem, b: &Self::Elem) -> Option<Self::Elem>;
}
...
pub trait MonoidSignature: IdentitySignature + AssociativeCompositionSignature {
    fn compose_list(self: &Rc<Self>, elems: Vec<impl Borrow<Self::Elem>>) -> Self::Elem {
        if elems.is_empty() {
            self.identity()
        } else {
    ...
    + RightCancellativeCompositionSignature
{
    ...
    fn int_pow(self: &Rc<Self>, a: &Self::Elem, n: &Integer) -> Self::Elem {
...
pub trait LeftGroupActionSignature<Group: GroupSignature, Set: SetSignature>: Signature {
    ...
    fn apply(self: &Rc<Self>, g: &Group::Elem, x: &Set::Elem) -> Set::Elem;
}
