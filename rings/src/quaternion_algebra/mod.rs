...
impl<Field: FieldSignature + CharacteristicSignature> QuaternionAlgebraStructure<Field> {
    pub fn base_field(&self) -> &Rc<Field> {
        &self.base
    }

    pub fn new(base: Rc<Field>, a: Field::Elem, b: Field::Elem) -> Rc<Self> {
...
impl<Field: FieldSignature> SemiModuleSignature<Field> for QuaternionAlgebraStructure<Field> {
    fn ring(self: &Rc<Self>) -> Rc<Field> {
        self.base.clone()
    }

    fn scalar_mul(self: &Rc<Self>, a: &Self::Elem, x: &Field::Elem) -> Self::Elem {
        let base = &self.base;
        QuaternionAlgebraElement {
            x: base.mul(x, &a.x),
            y: base.mul(x, &a.y),
...
impl<Field: FieldSignature> FreeModuleSignature<QuaternionAlgebraBasisCanonicalStructure, Field>
    for QuaternionAlgebraStructure<Field>
{
    fn basis_set(self: &Rc<Self>) -> Rc<QuaternionAlgebraBasisCanonicalStructure> {
    ...
    fn from_component(self: &Rc<Self>, b: &QuaternionAlgebraBasis, r: &Field::Elem) -> Self::Elem {
        let mut v = self.zero();
        match b {
...
impl<Field: CharZeroFieldSignature> CharZeroRingSignature for QuaternionAlgebraStructure<Field> {
    fn try_to_int(self: &Rc<Self>, a: &Self::Elem) -> Option<algebraeon_structures::Integer> {
