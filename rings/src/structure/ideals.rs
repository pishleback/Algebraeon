...
#[signature_meta_trait]
pub trait IdealsArithmeticSignature<Ring: RingSignature>:
    IdealsSignature<Ring> + SemiRingEqSignature
{
    /// The ideal generated by a list of elements
    fn generated_ideal(self: &Rc<Self>, elems: Vec<impl Into<Ring::Elem>>) -> Self::Elem {
        self.sum(
            &elems
                .into_iter()
    ...
    fn principal_ideal(self: &Rc<Self>, a: &Ring::Elem) -> Self::Elem;

    /// Does a contain b i.e. does a divide b
    fn contains_ideal(self: &Rc<Self>, a: &Self::Elem, b: &Self::Elem) -> bool;

    /// Does a contain x
    fn contains_element(self: &Rc<Self>, a: &Self::Elem, x: &Ring::Elem) -> bool {
...
pub trait PrincipalIdealsSignature<Ring: RingSignature>: IdealsSignature<Ring> {
    fn ideal_generator(self: &Rc<Self>, ideal: &Self::Elem) -> Ring::Elem;
}
    ...
    + UniqueFactorizationMonoidSignature<FactoredExponent = NaturalCanonicalStructure>
{
    ...
    fn padic_r_element_valuation(self: &Rc<Self>, prime: &Self::Elem, a: &Ring::Elem) -> Valuation {
    ...
    /// this is the largest power `k` such that `I^k` contains `a`
    fn padic_r_ideal_valuation(self: &Rc<Self>, prime: &Self::Elem, a: &Self::Elem) -> Valuation {
