    ...
    IdealsSignature<Ring, RingB>
{
    ...
    fn equal(&self, a: &Self::Set, b: &Self::Set) -> bool {
        self.contains_ideal(a, b) && self.contains_ideal(b, a)
    }

    /// Is the ideal the zero ideal
    fn is_zero(&self, a: &Self::Set) -> bool {
        self.equal(a, &self.zero_ideal())
    }
    ...
    /// Sum of many ideals
    fn sum(&self, ideals: Vec<impl Into<Self::Set>>) -> Self::Set {
        let mut total = self.zero_ideal();
        for i in ideals {
            total = self.add(&total, &i.into());
        }
        total
    }
    ...
    fn product(&self, ideals: Vec<impl Into<Self::Set>>) -> Self::Set {
        let mut total = self.unit_ideal();
        for i in ideals {
            total = self.mul(&total, &i.into());
        }
        total
    }
    ...
    fn nat_pow(&self, ideal: &Self::Set, n: &Natural) -> Self::Set {
        if *n == Natural::ZERO {
            self.unit_ideal()
...
>: IdealsArithmeticSignature<Ring, RingB>
{
    ...
    fn largest_prime_ideal_factor_power(
        ...
        debug_assert!(!self.equal(prime_ideal.ideal(), &self.unit_ideal()));
        ...
        while self.contains_ideal(&prime_ideal_to_the_k_plus_one, ideal) {
            ...
            prime_ideal_to_the_k_plus_one =
                self.mul(&prime_ideal_to_the_k_plus_one, prime_ideal.ideal());
        }
        k
    }
    ...
    fn padic_r_element_valuation(
        ...
        loop {
            ...
            k += 1;
            prime_to_the_k = self.mul(&prime_to_the_k, prime.ideal());
        }
    }
