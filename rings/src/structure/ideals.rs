    ...
    IdealsSignature<Ring> + SemiRingEqSignature
{
    ...
    fn generated_ideal(self: &Arc<Self>, elems: Vec<impl Into<Ring::Elem>>) -> Self::Elem {
        self.sum(
            &elems
                .into_iter()
    ...
    fn principal_ideal(self: &Arc<Self>, a: &Ring::Elem) -> Self::Elem;

    /// Does a contain b i.e. does a divide b
    fn contains_ideal(self: &Arc<Self>, a: &Self::Elem, b: &Self::Elem) -> bool;

    /// Does a contain x
    fn contains_element(self: &Arc<Self>, a: &Self::Elem, x: &Ring::Elem) -> bool {
...
pub trait PrincipalIdealsSignature<Ring: RingSignature>: IdealsSignature<Ring> {
    fn ideal_generator(self: &Arc<Self>, ideal: &Self::Elem) -> Ring::Elem;
}
    ...
    + UniqueFactorizationMonoidSignature<FactoredExponent = NaturalCanonicalStructure>
{
    ...
    /// this is the largest power `k` such that `I^k` contains `a`
    fn padic_r_ideal_valuation(self: &Arc<Self>, prime: &Self::Elem, a: &Self::Elem) -> Valuation {
