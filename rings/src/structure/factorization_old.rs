...
pub trait FactoredSignature: SetSignature {
    ...
    type Unfactored: Clone + Debug;
    /// A type used to hold the prime objects.
    type Prime: Clone + Debug;
    ...
    fn new_powers(&self, factor_powers: Vec<(Self::Prime, Natural)>) -> Self::Set {
        #[cfg(debug_assertions)]
        {
            let mut ps: Vec<&Self::Prime> = vec![];
    ...
    fn new_prime(&self, prime: Self::Prime) -> Self::Set {
    ...
    fn to_powers<'a>(&self, a: &'a Self::Set) -> Vec<(&'a Self::Prime, &'a Natural)> {
    ...
    fn into_powers(&self, a: Self::Set) -> Vec<(Self::Prime, Natural)> {
        debug_assert!(self.is_element(&a).is_ok());
    ...
    fn to_primes<'a>(&self, a: &'a Self::Set) -> Vec<&'a Self::Prime> {
        debug_assert!(self.is_element(a).is_ok());
        let mut factors = vec![];
    ...
    fn into_primes(&self, a: Self::Set) -> Vec<Self::Prime> {
        debug_assert!(self.is_element(&a).is_ok());
        let mut factors = vec![];
    ...
    fn to_prime_support<'a>(&self, a: &'a Self::Set) -> Vec<&'a Self::Prime> {
        debug_assert!(self.is_element(a).is_ok());
    ...
    fn into_prime_support(self, a: Self::Set) -> Vec<Self::Prime> {
        debug_assert!(self.is_element(&a).is_ok());
    ...
    fn object_one(&self) -> Self::Unfactored {
        self.object_product(vec![])
    }

    /// The product of two objects
    fn object_mul(&self, a: &Self::Unfactored, b: &Self::Unfactored) -> Self::Unfactored {
    ...
    fn object_equivalent(&self, a: &Self::Unfactored, b: &Self::Unfactored) -> bool {
    ...
    fn prime_object_equivalent(&self, a: &Self::Prime, b: &Self::Prime) -> bool {
    ...
    fn expanded_squarefree(&self, a: &Self::Set) -> Self::Unfactored {
        debug_assert!(self.is_element(a).is_ok());
        self.expanded(
            &self.new_powers(
    ...
    fn divisors<'a>(&'a self, a: &'a Self::Set) -> Box<dyn Iterator<Item = Self::Unfactored> + 'a> {
