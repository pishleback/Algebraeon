...
use std::{borrow::Cow, ops::Rem, rc::Rc, sync::Mutex};
...
impl MontgomeryModuloOddStructure {
    ...
    pub fn new_unchecked(n: u64) -> Rc<Self> {
...
pub struct MontgomeryModuloOddPrimeStructure {
    sup: Rc<MontgomeryModuloOddStructure>,
    inv_cache: Rc<Mutex<Option<Vec<u64>>>>,
}
...
impl MontgomeryModuloOddPrimeStructure {
    ...
    pub fn new_unchecked(n: u64) -> Rc<Self> {
        ...
        Self {
            sup,
            inv_cache: Rc::new(Mutex::new(None)),
        }
        .into()
    }
...
impl CountableSetSignature
    for MultiplicativeMonoidUnitsStructure<MontgomeryModuloOddPrimeStructure>
{
    fn generate_all_elements(self: Rc<Self>) -> impl Iterator<Item = Self::Elem> {
        1..self.monoid().sup.n
    }
