...
impl FactoringMonoidSignature for IntegerCanonicalStructure {
    fn factor_unchecked(self: &Rc<Self>, a: &Self::Elem) -> Factored<Integer, Natural> {
        if a == &Integer::ZERO {
            Factored::Zero
        } else {
...
impl CharZeroRingSignature for IntegerCanonicalStructure {
    fn try_to_int(self: &Rc<Self>, x: &Integer) -> Option<Integer> {
        Some(x.clone())
    }
}

impl AlgebraicIntegerRingSignature<RationalCanonicalStructure> for IntegerCanonicalStructure {
    fn anf(self: &Rc<Self>) -> Rc<RationalCanonicalStructure> {
        Rational::structure()
    }

    fn to_anf(self: &Rc<Self>, x: &Integer) -> Rational {
        Rational::from(x)
    }

    fn try_from_anf(self: &Rc<Self>, y: &Rational) -> Option<Integer> {
        Integer::try_from_rat(y)
    }

    fn integral_basis(self: &Rc<Self>) -> Vec<Integer> {
        vec![Integer::ONE]
    }
}
...
impl MultiplicativeMonoidSquareOpsSignature for IntegerCanonicalStructure {
    fn sqrt_if_square(self: &Rc<Self>, a: &Integer) -> Option<Integer> {
        a.sqrt_if_square().map(|n: Natural| Integer::from(n))
    }

    fn is_square(self: &Rc<Self>, a: &Integer) -> bool {
        a.is_square()
    }
}
...
impl FreeRingSignature for IntegerCanonicalStructure {
    ...
    fn free_generators(self: &Rc<Self>) -> HashSet<Self::Generator> {
        HashSet::new()
    }
}
